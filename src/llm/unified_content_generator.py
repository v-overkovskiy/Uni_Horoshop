...
Полное описание товара (как есть): {full_description}
Локаль: {locale}

🚫 КРИТИЧЕСКИ ВАЖНО - НАРУШЕНИЕ ВЛЕЧЕТ ОШИБКУ:
- ❗❗❗ ЕСЛИ есть "Полное описание товара" — ТЫ ОБЯЗАН проанализировать его и выделить ВАЖНЫЕ факты для данного товара!
- ❗❗❗ Используй факты из полного описания для формирования улучшенного описания и блока преимуществ!
- ❗❗❗ ЛЮБОЙ важный факт из полного описания — это РЕАЛЬНАЯ информация о товаре, ты ДОЛЖЕН включить его в текст!
- ⛔⛔⛔ КРИТИЧНО: ЗАПРЕЩЕНО ИЗМЕНЯТЬ РАЗМЕРЫ И ЦИФРЫ! Если в данных указан размер 180/200 см — ИСПОЛЬЗУЙ 180/200 см, НЕ ИЗМЕНЯЙ!
- ⛔⛔⛔ ЗАПРЕЩЕНО выдумывать цифры, размеры, веса, которых НЕТ во входных данных!
...
- 🚫 КРИТИЧНО: ЗАПРЕЩЕНО ДОБАВЛЯТЬ ИНФОРМАЦИЮ, КОТОРАЯ НЕ УКАЗАНА В ХАРАКТЕРИСТИКАХ
- 🚫 ЗАПРЕЩЕНО: выдумывать свойства, функции, назначение, применения, которых нет в исходных данных
- 🚫 ЗАПРЕЩЕНО: добавлять слова типа "для сервировки", "для украшения", "для подарка" ЕСЛИ ЭТОГО НЕТ В ХАРАКТЕРИСТИКАХ
...
- 📊 ОБЯЗАТЕЛЬНО включай конкретные факты из исходника: размеры, вес, материалы, свойства
- 🔍 ИЗВЛЕКАЙ ВСЕ ЦИФРЫ: вес (кг/г), размеры (см/мм), толщина, объем, количество
- 🔍 ИЗВЛЕКАЙ ВСЕ СВОЙСТВА: прочность, безопасность, покрытия, эффекты
- 🔍 ИЗВЛЕКАЙ ВСЕ МАТЕРИАЛЫ: названия материалов, их особенности, цифры
...
1. ОПИСАНИЕ (строка с HTML тегами, СТРОГО 6 предложений в 2 абзацах):
   - ❗❗❗ КРИТИЧНО: Абзац 1 = РОВНО 3 предложения, Абзац 2 = РОВНО 3 предложения!
   - ❗❗❗ ОБЯЗАТЕЛЬНО: проанализируй "Полное описание товара" и выдели ВАЖНЫЕ факты!
   - ❗❗❗ ПРАВИЛО: извлекай из полного описания важные факты (батарея LR44, календарь, часы 12/24, сигнализация, память lap и т.д.) — УКАЖИ их в тексте!
   - Абзац 1 (СТРОГО 3 предложения): Назначение, кому подходит, основные свойства
   - 🚫 НЕ ДОБАВЛЯЙ функции "для сервировки", "для сервірування", "для украшения", "для жарки", "для перемешивания" ЕСЛИ ИХ НЕТ В ХАРАКТЕРИСТИКАХ ИЛИ В "ДОПОЛНИТЕЛЬНЫХ ФАКТАХ"
   - Абзац 2 (СТРОГО 3 предложения): Преимущества, результаты, практические детали
   ...
   - 📊 КРИТИЧНО: Анализируй полное описание товара и включай ВАЖНЫЕ факты, БЕЗ ИСКЛЮЧЕНИЙ:
   - ✅ ПРИОРИТЕТ #1: Все важные факты из "Полного описания товара" — ОБЯЗАТЕЛЬНО использовать!
   - Все размеры (развернутый, в сложенном виде, длина, ширина, толщина)
   - Вес, объем, количество
   - Материал и его покрытие
   - Прочность, износостойкость
   - Эффекты (массажный, антипригарный)
   - Комплектация (сумка, отверстие для подвешивания)
   - Легкость очистки, не впитывает запахи
   - Особенности ручки (не нагревается, не скользит)
   - Подходит для (посуды с антипригарным покрытием, посудомойки)
   - Термоизоляция, не пропускает холод
   - ✅ ПРАВИЛО: если важный факт есть в характеристиках ИЛИ в полном описании — ОБЯЗАТЕЛЬНО укажи его в описании!
   - Примеры фактов: "размер 34,5×8,5 см", "вес до 1 кг", "размер в сложенном виде 90×40 см", "нельзя прокусить/порвать", "пищевая пленка", "харчова плівка", "ребристая поверхность", "массажный эффект", "зносостойкий", "травмобезопасный", "отверстие для подвешивания", "не впитывает запахи", "ручка не нагревается и не скользит", "легко очищается", "термоизоляционный эффект", "не пропускает холод"

2. ПРЕИМУЩЕСТВА (СТРЕМИСЬ К 6 КАРТОЧКАМ, МИНИМУМ 3):
   - ❗❗❗ КРИТИЧЕСКИ ВАЖНО: анализируй "Полное описание товара" и выделяй важные факты для преимуществ!
   - ❗❗❗ ПРАВИЛО: каждое преимущество = важный факт из полного описания!
   - ИДЕАЛЬНО: 6 уникальных преимуществ с конкретными фактами из полного описания
   - ДОПУСТИМО: 3-5, если важных фактов недостаточно
   - 🚫 КРИТИЧНО: ЗАПРЕЩЕНО добавлять свойства, которых нет в характеристиках ИЛИ в полном описании
   - НЕ используй generic-фразы: "высокое качество", "эффективный результат"
   - Каждое преимущество должно быть уникальным и полезным
   - ✅ ПРИОРИТЕТ #1: Анализируй полное описание и выделяй важные факты для преимуществ!
   - 📊 КРИТИЧНО: Включай КОНКРЕТНЫЕ важные факты из полного описания: батарея LR44, календарь, часы 12/24, сигнализация, память lap и т.д.!
   - Обязательные факты для включения (если есть в характеристиках ИЛИ в "Дополнительных фактах"): вес, размер в сложенном виде, прочность, покрытие (пищевая пленка), эффекты (массажный), комплектация (сумка, отверстие), износостойкость, очистка, особенности ручки, антипригарность, термоизоляция, не пропускает холод
   - Примеры: "Вага до 1 кг робить його легким для перенесення", "Компактний розмір 90×40 см у складеному вигляді", "Массажний ефект для розвитку дитини", "Харчова плівка на поверхні", "Ребриста поверхня", "Не можна прокусити або порвати", "Зносостійкий матеріал", "Травмо-безпечний", "Легко очищається, не вбирає запахи", "Термоізоляційний ефект", "Не пропускає холод від підлоги"
...
ПРАВИЛЬНЫЙ ПРИМЕР ОПИСАНИЯ (КРИТИЧНО: 6 предложений = 3+3):
...
❌ Менее 6 предложений
❌ Абзац с НЕ РОВНО 3 предложениями

ФОРМАТ ВЫВОДА (строго JSON):
{{
  "description": "<p>первый абзац с РОВНО 3 предложениями</p><p>второй абзац с РОВНО 3 предложениями</p>",
...
"""
    ...
    async def generate_unified_content(self, product_facts: Dict[str, Any], locale: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Извлекаем полное описание товара как есть
            full_description = product_facts.get('full_description', '')
            if not full_description:
                full_description = "(описание отсутствует)"
            ...
            prompt_data = {
                ...
                'full_description': full_description,  # Полное описание как есть
                'locale': locale,
...
- 🚫 ЗАПРЕЩЕНО выдумывать назначение или функции, которых НЕТ в характеристиках
- Определи ТОЛЬКО базовое назначение из названия (НЕ добавляй функции из "особенностей")
- НЕ используй общие фразы типа "уход за кожей", "для украшения", "для подарка"
- НЕ добавляй слова "для сервировки", "для жарки", "для перемешивания" ЕСЛИ ЭТО НЕ УКАЗАНО В ХАРАКТЕРИСТИКАХ
- Будь МИНИМАЛИСТИЧНЫМ: просто базовое назначение из названия
- БЕЗ пояснений
